...
function formatUnit(val: number, unit?: string): string {
  ...
  if (unit === 't/qtr') return `${Math.round(val).toLocaleString()}t`;
...
const DrillDownPanel = ({ variable, gdiResult, goldSpot, timeRange, onClose }: DrillDownPanelProps) => {
  ...
  const { chartData, varDomain, goldDomain, stats } = useMemo(() => {
    ...
    const std = allValues.length > 1 ? Math.sqrt(allValues.reduce((a, b) => a + (b - mean) ** 2, 0) / allValues.length) : 0;
  ...
  return (
    <div className="rounded-lg border border-gold/30 bg-card overflow-hidden animate-fade-in">
      {/* Header */}
      <div className="flex items-center justify-between px-4 sm:px-5 py-3 border-b border-card-border">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: lineColor }} />
          <h3 className="font-display text-sm sm:text-base text-foreground truncate">{variable.name}</h3>
          <span className="text-[10px] sm:text-xs text-muted-foreground font-mono hidden sm:inline">{variable.id}</span>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 lg:grid-cols-4 gap-0 divide-y md:divide-y-0 md:divide-x divide-card-border">
        {/* Column 1: Chart (50% on desktop, 60% on tablet) */}
        <div className="p-3 sm:p-4 md:col-span-3 lg:col-span-2">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] sm:text-xs text-muted-foreground">Historical ({timeRange})</span>
            <label className="flex items-center gap-1.5 text-[10px] text-muted-foreground cursor-pointer">
              <input type="checkbox" checked={showGoldOverlay} onChange={() => setShowGoldOverlay(v => !v)} className="w-3 h-3 rounded" />
          ...
          <ResponsiveContainer width="100%" height={280}>
            <ComposedChart data={chartData} margin={{ top: 10, right: 10, bottom: 10, left: 10 }}>
              ...
              <XAxis dataKey="dateTs" type="number" domain={['dataMin', 'dataMax']} tickFormatter={formatXTick} stroke="#8A8D96" tick={{ fontSize: 9, fill: '#8A8D96' }} axisLine={{ stroke: '#1E2230' }} />
              <YAxis yAxisId="var" orientation="left" domain={varDomain as [number, number]} tickFormatter={(v: number) => v.toFixed(unitStr === '%' ? 2 : 0)} stroke={lineColor} tick={{ fontSize: 9, fill: lineColor }} axisLine={{ stroke: '#1E2230' }} width={55} />
              {showGoldOverlay && (
                <YAxis yAxisId="gold" orientation="right" domain={goldDomain as [number, number]} tickFormatter={(v: number) => `$${v.toLocaleString()}`} stroke="#C9A84C" tick={{ fontSize: 9, fill: '#C9A84C' }} axisLine={{ stroke: '#1E2230' }} width={60} strokeOpacity={0.4} />
              )}
              {meta?.regimeBands?.map((band, i) => (
                <ReferenceArea key={i} x1={new Date(band.startDate).getTime()} x2={new Date(band.endDate).getTime()} yAxisId="var" fill={band.color === 'bullish' ? 'rgba(61,220,132,0.06)' : 'rgba(255,82,82,0.06)'} fillOpacity={1} label={{ value: band.label, position: 'insideTopLeft', fill: '#8A8D96', fontSize: 8 }} />
              ))}
              <Line yAxisId="var" dataKey="value" stroke={lineColor} strokeWidth={2} dot={false} connectNulls isAnimationActive={false} />
              {showGoldOverlay && <Line yAxisId="gold" dataKey="gold" stroke="#C9A84C" strokeWidth={1.5} strokeOpacity={0.4} dot={false} connectNulls isAnimationActive={false} />}
              <Tooltip
                contentStyle={{ backgroundColor: '#151820', border: '1px solid #1E2230', borderRadius: 8, fontSize: 11 }}
                ...
                formatter={(val: number, name: string) => {
                  ...
                  return [formatUnit(val, unitStr), variable.name];
                }}
              />
            </ComposedChart>
          </ResponsiveContainer>
        ...
        {/* Column 2+3 stacked on mobile, side-by-side on tablet+ */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:col-span-2 lg:col-span-2 divide-y sm:divide-y-0 sm:divide-x divide-card-border">
          {/* Column 2: Stats */}
          <div className="p-3 sm:p-4">
            <h4 className="text-[10px] sm:text-xs text-muted-foreground mb-3 uppercase tracking-wider">Statistical Breakdown</h4>
            {stats && (
              <div className="space-y-1.5 text-[11px] sm:text-xs">
                <StatRow label="Current value" value={formatUnit(variable.currentValue, unitStr)} />
                <StatRow label="10-year mean" value={formatUnit(stats.mean, unitStr)} />
                <StatRow label="10-year std dev" value={stats.std.toFixed(2)} />
                <StatRow label="Z-score (raw)" value={`${variable.zScore > 0 ? '+' : ''}${variable.zScore.toFixed(3)}`} />
                <StatRow label="Z-score (adjusted)" value={`${variable.adjustedZScore > 0 ? '+' : ''}${variable.adjustedZScore.toFixed(3)}`} highlight={variable.adjustedZScore > 0 ? 'bullish' : variable.adjustedZScore < 0 ? 'bearish' : undefined} />
                <StatRow label="52-week high" value={formatUnit(stats.high52, unitStr)} />
                <StatRow label="52-week low" value={formatUnit(stats.low52, unitStr)} />
                <StatRow label="Percentile (10yr)" value={`${stats.percentile.toFixed(0)}th`} />
                <StatRow label="Corr w/ gold (52w)" value={`${stats.corr > 0 ? '+' : ''}${stats.corr.toFixed(2)}`} />
            ...
            {meta && (
              <div className="mt-4 pt-3 border-t border-card-border">
                <h4 className="text-[10px] sm:text-xs text-muted-foreground mb-2 uppercase tracking-wider">What this means</h4>
          ...
          <div className="p-3 sm:p-4">
            {meta && (
              <div className="space-y-4 sm:space-y-5">
                <div>
                  <h4 className="text-[10px] sm:text-xs text-muted-foreground mb-2 uppercase tracking-wider">Data Source</h4>
                  <div className="space-y-1 text-[11px] sm:text-xs">
                    {meta.fredUrl && (
                      <a href={meta.fredUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-index-blue hover:text-index-blue/80 transition-colors">
                        <ExternalLink className="w-3 h-3 flex-shrink-0" />{meta.id}
                      </a>
                    )}
                ...
                <div>
                  <h4 className="text-[10px] sm:text-xs text-muted-foreground mb-2 uppercase tracking-wider">Why this matters for gold</h4>
                  <div className="space-y-2">
                    {meta.evidenceSources.map((src, i) => (
                      <a key={i} href={src.url} target="_blank" rel="noopener noreferrer" className="block group">
                        <p className="text-[11px] sm:text-xs text-index-blue group-hover:text-index-blue/80 transition-colors flex items-center gap-1">
                          <ExternalLink className="w-2.5 h-2.5 flex-shrink-0" />{src.title}
                        </p>
                        <p className="text-[9px] sm:text-[10px] text-muted-foreground/60 ml-4 italic">"{src.quote}"</p>
                      </a>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  <h4 className="text-[10px] sm:text-xs text-muted-foreground mb-2 uppercase tracking-wider">What influences this variable</h4>
                  <div className="space-y-1.5">
                    {meta.contextLinks.map((link, i) => (
                      <a key={i} href={link.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-[11px] sm:text-xs text-index-blue hover:text-index-blue/80 transition-colors">
                        <ExternalLink className="w-2.5 h-2.5 flex-shrink-0" />{link.title}
                      </a>
                    ))}
                  </div>
                </div>
