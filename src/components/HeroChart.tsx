...
const HeroChart = ({
  ...
  const { chartData, goldYDomain, gdiYDomain, bullishDivs, bearishDivs } = useMemo(() => {
    ...
    // Sample to ~500 historical points max for performance
    ...
    filteredIndices.forEach((idx, i) => {
      ...
      // If no gold spot, try futures as fallback
      const goldPrice = spot ?? futures;

      if (goldPrice !== undefined) {
        minGold = Math.min(minGold, goldPrice);
        maxGold = Math.max(maxGold, goldPrice);
      }
      minGdi = Math.min(minGdi, gdi);
      maxGdi = Math.max(maxGdi, gdi);

      data.push({
        date,
        dateTs: new Date(date).getTime(),
        goldSpot: spot ?? futures,
        goldFutures: futures,
        gdi,
        isForecast: false,
      });
    ...
    forecastPoints.forEach(fp => {
      ...
      data.push({
        date: fp.date,
        dateTs: new Date(fp.date).getTime(),
        bullForecast: fp.bull,
        baseForecast: fp.base,
        bearForecast: fp.bear,
        evForecast: fp.ev,
        isForecast: true,
      });
    });
  ...
  // Bank consensus points for scatter
  const bankPoints = useMemo(() => {
    if (!showBankConsensus) return [];
    return BANK_CONSENSUS.map(b => ({
      dateTs: new Date(b.date).getTime(),
      goldSpot: b.price,
      bank: b.bank,
    }));
  }, [showBankConsensus]);
        ...
        <div className="flex items-center gap-4">
          <label className="flex items-center gap-1.5 text-xs text-muted-foreground cursor-pointer">
            <input
              type="checkbox"
              checked={showFutures}
              onChange={onToggleFutures}
              className="rounded border-card-border bg-card accent-futures-purple w-3.5 h-3.5"
            />
          ...
          <label className="flex items-center gap-1.5 text-xs text-muted-foreground cursor-pointer">
            <input
              type="checkbox"
              checked={showBankConsensus}
              onChange={onToggleBankConsensus}
              className="rounded border-card-border bg-card accent-gold w-3.5 h-3.5"
            />
            Bank Consensus
          </label>
        </div>
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
            <span className="flex items-center gap-1">
              <span className="w-3 h-0.5 bg-gold inline-block rounded" /> Gold
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-0.5 bg-index-blue inline-block rounded" /> GDI
            </span>
          </div>
        </div>
      </div>
        ...
        <ResponsiveContainer width="100%" height={500}>
          <ComposedChart data={chartData} margin={{ top: 20, right: 60, bottom: 20, left: 20 }}>
            ...
            <XAxis
              ...
              stroke="#8A8D96"
              tick={{ fontSize: 10, fill: '#8A8D96' }}
              axisLine={{ stroke: '#1E2230' }}
              tickLine={{ stroke: '#1E2230' }}
            />

            <YAxis
              yAxisId="gold"
              orientation="left"
              domain={goldYDomain as [number, number]}
            ...
            <YAxis
              yAxisId="gdi"
              orientation="right"
              domain={gdiYDomain as [number, number]}
              tickFormatter={(v: number) => v.toFixed(1)}
              stroke="#5C9CE6"
              tick={{ fontSize: 10, fill: '#5C9CE6' }}
              axisLine={{ stroke: '#1E2230' }}
              tickLine={{ stroke: '#1E2230' }}
              width={45}
            />
            ...
            <ReferenceLine
              ...
              strokeDasharray="4 4"
              strokeOpacity={0.5}
              label={{ value: 'Today', position: 'top', fill: '#C9A84C', fontSize: 10 }}
            />

            {/* Fan area between bull and bear forecast */}
            <Area
              yAxisId="gold"
              dataKey="bullForecast"
              stroke="none"
            ...
            <Line
              ...
              dataKey="bullForecast"
              stroke="#3DDC84"
              strokeWidth={Math.max(1, probs.bull * 4)}
              dot={{ r: 4, fill: '#3DDC84', stroke: '#0D0F12', strokeWidth: 2 }}
              connectNulls={false}
              isAnimationActive={false}
            />
            <Line
              ...
              stroke="#C9A84C"
              strokeWidth={Math.max(1, probs.base * 4)}
              dot={{ r: 4, fill: '#C9A84C', stroke: '#0D0F12', strokeWidth: 2 }}
              connectNulls={false}
              isAnimationActive={false}
            />
            <Line
              ...
              dataKey="bearForecast"
              stroke="#FF5252"
              strokeWidth={Math.max(1, probs.bear * 4)}
              dot={{ r: 4, fill: '#FF5252', stroke: '#0D0F12', strokeWidth: 2 }}
              connectNulls={false}
              isAnimationActive={false}
            />
            ...
            <Line
              ...
              strokeWidth={2}
              strokeDasharray="8 4"
              dot={{ r: 5, fill: '#C9A84C', stroke: '#0D0F12', strokeWidth: 2 }}
              connectNulls={false}
              isAnimationActive={false}
            />

            {/* Bank consensus diamonds */}
            {showBankConsensus && (
              <Scatter
                yAxisId="gold"
                data={bankPoints}
            ...
            <Tooltip
              content={<HeroChartTooltip probs={probs} showBankConsensus={showBankConsensus} todayTs={todayTs} />}
              cursor={{ stroke: '#C9A84C', strokeOpacity: 0.2, strokeDasharray: '4 4' }}
            />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
